...
function ResultMovie({ id, title, releaseDate, rating, overview, genres, poster }: MovieComponentInterface) {
			...
			<MovieContainer>
				...
				<MovieDescription>
					...
					<MovieOverview>
						...
						{isOverviewExpanded ? overview : `${overview.substring(0, 150)}${overview.length > 150 ? "..." : ""}`}
						{/* Display "Read More" button only if overview is too long */}
						{overview.length > 150 && (
							<ReadMoreButton onClick={toggleOverview}>{isOverviewExpanded ? "Read Less" : "Read More"}</ReadMoreButton>
						)}
					</MovieOverview>
				</MovieDescription>
				<WatchTrailerButton onClick={() => handleTrailer(id)}>Watch trailer</WatchTrailerButton>
				<MovieDetails>
