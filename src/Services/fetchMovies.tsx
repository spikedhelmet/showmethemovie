...
async function fetchMovies(input: string, fetchType: string, page: number = 1): Promise<MovieComponentInterface[]> {
	const listUrl = `https://api.themoviedb.org/3/movie/${input}?&include_adult=false&language=en-US&page=${page}`;
	const searchUrl = `https://api.themoviedb.org/3/search/movie?query=${input}&include_adult=false&language=en-US&page=${page}`;
	...
	try {
		...
		const mappedMoviesData = await Promise.all(
			allMovies.map(async (movie: FetchedMovieInterface) => {
				const movieDetails: MovieDetailsInterface = await fetchDataById(movie.id);
